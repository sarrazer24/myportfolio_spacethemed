...
export default function CertificatesSection() {
  ...
  const currentSection = timelineSections.find(s => s.key === activeSection) || timelineSections[0]
        ...
        <div className="flex flex-col md:flex-row gap-8">
          ...
          <ol className="relative border-l-2 border-cyan-700 md:w-64 w-full mb-8 md:mb-0">
            {timelineSections.map((section, idx) => (
              ...
              >
                <div className="absolute -left-4 flex items-center justify-center w-6 h-6 rounded-full border-2 border-cyan-700 bg-gray-900 transition-all
                  "
                  style={{
                    borderColor: activeSection === section.key ? "#22d3ee" : "#155e75",
                    boxShadow: activeSection === section.key ? "0 0 0 2px #22d3ee" : "none",
                    transform: activeSection === section.key ? "scale(1.15)" : "scale(1)"
                  }}
                />
                <div className={`pl-2 transition-colors border-l-2 ${activeSection === section.key ? "border-cyan-400 text-cyan-300" : "border-transparent text-gray-400 group-hover:text-cyan-200"}`}>
          ...
          <div className="flex-1">
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {currentSection.data.length === 0 ? (
                ...
                currentSection.data.map((course, idx) => (
                  <motion.div
                    key={idx}
